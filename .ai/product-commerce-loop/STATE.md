...
- Guest checkout retries now retain an idempotency key in session storage for an unchanged cart across page refreshes and retryable failures; the cart remains in local storage until a confirmed successful order response. Production serves `ac880c1`.
