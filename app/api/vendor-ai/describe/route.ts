...
export async function POST(req: NextRequest) {
  ...
  if (!anthropic) return NextResponse.json({ error: 'AI is not configured yet.' }, { status: 503 })
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (ALLOWED_IMG.includes(ct) && buf.byteLength <= 3 * 1024 * 1024) {
          imageBlock = { type: 'image', source: { type: 'base64', media_type: ct as 'image/webp', data: buf.toString('base64') } }
        }
      }
  ...
  const content: Anthropic.ContentBlockParam[] = imageBlock ? [imageBlock, { type: 'text', text: promptText }] : [{ type: 'text', text: promptText }]

  try {
    const res = await anthropic.messages.create({
      model: imageBlock ? MODELS.vision : MODELS.fast,
      max_tokens: 80,
      system: SYSTEM,
    ...
    return NextResponse.json({ description: text, from_photo: !!imageBlock })
  } catch (err) {
