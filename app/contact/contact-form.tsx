...
export function ContactForm() {
  ...
  return <form onSubmit={submit} className="space-y-4 rounded-3xl border border-white/10 bg-white/5 p-5">
    <label className="block text-sm text-white/75">What do you need help with?<select name="intent" required className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-[#111113] px-3 text-white">{intents.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
    <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm text-white/75">Name<input name="name" required minLength={2} maxLength={100} autoComplete="name" className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-[#111113] px-3 text-white" /></label><label className="block text-sm text-white/75">Email<input name="email" required type="email" maxLength={254} autoComplete="email" className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-[#111113] px-3 text-white" /></label></div>
    <label className="block text-sm text-white/75">Subject<input name="subject" required minLength={4} maxLength={140} className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-[#111113] px-3 text-white" /></label>
    <label className="block text-sm text-white/75">Order or application reference (if relevant)<input name="relatedReference" maxLength={80} className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-[#111113] px-3 text-white" /></label>
    ...
    <button disabled={state.loading} className="min-h-12 rounded-xl bg-[#F5A623] px-5 font-semibold text-black disabled:opacity-60">{state.loading ? 'Saving…' : 'Send request'}</button>
  </form>
}
