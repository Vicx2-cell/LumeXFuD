...
export default function GroupOrderPage() {
  ...
  return (
    <Shell>
      ...
      <div className="h-32" />
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[#0A0A0B]/95 px-4 pb-[calc(16px+env(safe-area-inset-bottom))] pt-3 backdrop-blur">
...
function ProductPicker({ picker, selected, note, required, optional, complete, busy, onToggle, onNote, onClose, onSave }: { picker: { menu: MenuItem; editing?: GroupItem }; selected: string[]; note: string; required: MenuAddon[]; optional: MenuAddon[]; complete: boolean; busy: boolean; onToggle: (addon: MenuAddon) => void; onNote: (value: string) => void; onClose: () => void; onSave: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70" onClick={onClose}><div className="max-h-[88dvh] w-full max-w-md overflow-y-auto overscroll-contain rounded-t-2xl border border-white/10 bg-[#111113] p-5 pb-0" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-lg font-semibold">{picker.menu.name}</h2><p className="text-sm text-amber-300">{money(picker.menu.price_kobo)}</p></div><button onClick={onClose} aria-label="Close product options" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/10"><X className="h-4 w-4" /></button></div><div className="mt-5 space-y-5">{choices('Required choice', required, true)}{choices('Optional extras', optional, false)}<div><label htmlFor="group-item-note" className="mb-2 block text-xs uppercase text-white/55">Item note</label><textarea id="group-item-note" value={note} onChange={(event) => onNote(event.target.value.slice(0, 200))} rows={3} className="lx-field w-full resize-none px-3 py-2" placeholder="Optional note for the vendor" /></div></div><div className="sticky bottom-0 -mx-5 mt-5 bg-[#111113] px-5 pb-[calc(16px+env(safe-area-inset-bottom))] pt-3"><button onClick={onSave} disabled={!complete || busy} className="lx-btn-amber min-h-13 w-full disabled:opacity-50">{complete ? `${picker.editing ? 'Save changes' : 'Add to group'} - ${money(chosenTotal)}` : 'Choose 1 required option'}</button></div></div></div>
}
