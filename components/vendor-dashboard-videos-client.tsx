...
export function VideosClient() {
  ...
  const load = useCallback(async (state: StateTab) => {
    ...
    setError('')
    try {
      const [quotaRes, listRes, staleRes] = await Promise.all([
              ...
              return (
                <article key={item.id} className="lx-surface p-3 flex gap-3">
                  ...
                  <div className="h-20 w-20 overflow-hidden rounded-xl bg-white/5 shrink-0">
                    {thumb ? <img src={thumb} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full w-full place-items-center text-xs text-white/30">No media</div>}
                  </div>
