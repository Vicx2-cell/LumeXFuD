        ...
        <section className="lx-surface p-4 space-y-4">
          ...
          {summary && (
            <p className="text-xs text-white/45">
              Revenue today: {formatPrice(summary.revenue_today_kobo)}
            </p>
          )}
        </section>
                  ...
                  {recent.map((order) => (
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        <p className="tabular-nums text-xs text-white/30">{formatPrice(order.total_amount)}</p>
                      </div>
    ...
    >
      ...
      <button onClick={() => setOpen((value) => !value)} aria-expanded={open} className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left">
        ...
        <div className="shrink-0 text-right">
          <p className="tabular-nums text-sm font-semibold text-white">{formatPrice(order.total_amount)}</p>
          <p className="text-[10px] uppercase tracking-wide text-white/30">{order.delivery_type}</p>
        </div>
      ...
      <div className="flex items-center justify-end gap-2 px-3 pb-3 pt-1">
        {order.status === 'PENDING' && (
          <>
            ...
            <button
              onClick={() => void act(() => onUpdate(order.id, 'VENDOR_ACCEPTED'))}
              disabled={busy}
