...
- Vendor and rider onboarding now supports both applicant-led registration (`/apply/vendor`, `/apply/rider`) and admin-assisted pending-account creation. Self-applicants verify their own contact channels and receive a one-time temporary PIN; admin-assisted creation no longer requires the admin to impersonate the applicant's OTP/email challenge. Neither path activates the account.
- Migration `155_partner_verification_assurance.sql` separates evidence verification from final approval. Admins can record a complete vendor premises/food-safety/payout/test-order inspection or rider identity/guarantor/vehicle/safety/training/test-delivery inspection. The evidence, notes, inspector and timestamp are retained; only a super-admin can give final approval and activation. Failed or incomplete checks remain inactive.
- The customer landing page now has a deliberately authored warm daylight palette while retaining the cinematic dark hero, instead of mechanically reversing white utilities. The group-order action bar and item-option submit area now remain visible above mobile safe areas and wrap long actions instead of clipping.
- The unused Super Admin Premium Billing console and its privileged API were removed. Customer-facing premium code remains separate and unchanged.
...
- 137 Vitest files / 892 tests pass, covering authz, order creation, cart contracts, delivery pricing, menu add-ons, payment-webhook idempotency, refunds, wallets, promotions, promo-fund locking/reconciliation, virtual-account boundaries, onboarding assurance, RLS coverage contracts, feeds, email flows and operations.
...
- After deployment, run one vendor and one rider through pending → verified → super-admin approved in production; confirm ordinary admins cannot perform verification or final approval.
...
- The GitHub workflow now enforces test, strict TypeScript, ESLint and production build before optional Vercel deployment. A migration dry-run and deployment-environment validation gate remain missing.
