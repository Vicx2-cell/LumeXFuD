...
export async function createReport(postId: string, reportType: string, reason: string) {
  ...
  if ((postAuthor as { is_system_account?: boolean } | null)?.is_system_account || (postAuthor as { official_badge_kind?: string } | null)?.official_badge_kind === 'official') {
    throw new Error('Official LumeX Fud posts cannot be reported here')
  }
  const now = new Date().toISOString()
