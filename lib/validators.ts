...
export const orderStatusInput = z.object({
  ...
  estimated_prep_minutes: z.number().int().min(3).max(45).optional(),
  estimated_delivery_minutes: z.number().int().min(0).max(30).optional(),
  estimate_reason: z.string().trim().min(5).max(160).optional(),
