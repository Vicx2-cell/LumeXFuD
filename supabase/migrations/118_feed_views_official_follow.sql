-- Reliable qualified views and the automatically followed official account.

CREATE OR REPLACE FUNCTION public.increment_post_qualified_view(p_post_id UUID)
RETURNS INTEGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE next_count INTEGER;
BEGIN
  UPDATE posts SET view_count = view_count + 1 WHERE id = p_post_id RETURNING view_count INTO next_count;
...
WHERE official.system_account_key = 'lumex_fud' AND profile.id <> official.id
ON CONFLICT (follower_profile_id, followed_profile_id) DO NOTHING;

CREATE OR REPLACE FUNCTION public.auto_follow_lumex_official()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE official_id UUID;
BEGIN
  SELECT id INTO official_id FROM social_profiles WHERE system_account_key = 'lumex_fud' LIMIT 1;
...
CREATE TRIGGER social_profiles_auto_follow_lumex AFTER INSERT ON social_profiles
FOR EACH ROW EXECUTE FUNCTION public.auto_follow_lumex_official();
