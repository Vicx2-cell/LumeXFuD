...
test.describe('commerce storefront browser flow', () => {
  for (const viewport of viewports) {
    test(`storefront product sheet and cart at ${viewport.label}`, async ({ page }, testInfo) => {
      ...
      await page.getByRole('heading', { name: 'Consent' }).scrollIntoViewIfNeeded()
      const payAction = page.getByRole('button', { name: /^Pay /i })
      await expect(payAction).toBeVisible()
      await expect(payAction).toBeDisabled()
      const [consentBox, payBox] = await Promise.all([terms.boundingBox(), payAction.boundingBox()])
      expect(consentBox).not.toBeNull()
      expect(payBox).not.toBeNull()
      expect(payBox!.y).toBeGreaterThanOrEqual(consentBox!.y + consentBox!.height)
